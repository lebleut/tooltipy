...
if(!empty($adv_options['bt_kw_supported_plugins'])){
	...
	if($options_supported_plgs['bbpress']){//if bbpress option checked
		add_filter('bbp_get_reply_content','kttg_filter_posttype',100);
	}
}else{
		...
		if(!empty($adv_options['bt_kw_supported_plugins'])){
			...
			if($options_supported_plgs['wooc']){//if wooc option checked
				if(get_post_type()=='product'){
					add_filter('the_content','kttg_filter_posttype',100);
				}		
				add_filter('woocommerce_short_description','kttg_filter_posttype',100);
			}
		}
	}
});
